...
1. day1 5시간
   ...
   -> 처음엔 기본적인 예외만 처리하자!
2. day2 5시간
   - 프로모션 할인, 멤버십 할인, 재고 관리 이 중에서 뭐부터 할 지 고민이 됨
   - 4시간 15분 지났을 때 기능 구현 완성
     - 리팩토링 해야함
3. day3
   - 리팩토링 하면서 드는 생각
   - 분리만 하면 끝이 아니라 변화에 유연하게 대처하고 싶어 리팩토링 하려니까 매우 어려워졌다.
     - 예를 들면 변수명이 변화가 되었을 때도 대처가 되게끔.
